...
title: SpineColors
target: Clay (Web)
permalink: api-docs/clay-web/ceramic/SpineColors/
---
...
|------|------|---------|-------------|
| `spine` | [Spine](/api-docs/clay-web/ceramic/Spine/) | | The Spine instance to extract colors from  |
| `slots` | [Array](/api-docs/clay-web/Array/)<[String](/api-docs/clay-web/String/)> | | Array of slot names to extract colors from  |
| `result` | [Array](/api-docs/clay-web/Array/)<[Color](/api-docs/clay-web/ceramic/Color/)> | *(optional)* | Optional array to store results in. If provided, it will be reused; otherwise a new array is created  |

| Returns | Description |
|---------|-------------|
| [Array](/api-docs/clay-web/Array/)<[Color](/api-docs/clay-web/ceramic/Color/)> | Array of Color values corresponding to each slot. Slots that don't exist or have no color will have Color.NONE * ```haxe var spine = new Spine(); spine.load(spineData); * // Extract colors from specific slots var slotNames = ["head", "body", "weapon"]; var colors = SpineColors.extractColors(spine, slotNames); * for (i in 0...slotNames.length) { trace('${slotNames[i]} color: ${colors[i].toHex()}'); } ``` |
...
| Name | Type | Default | Description |
|------|------|---------|-------------|
| `spine` | [Spine](/api-docs/clay-web/ceramic/Spine/) | | The Spine instance to extract dark colors from  |
| `slots` | [Array](/api-docs/clay-web/Array/)<[String](/api-docs/clay-web/String/)> | | Array of slot names to extract dark colors from  |
| `result` | [Array](/api-docs/clay-web/Array/)<[Color](/api-docs/clay-web/ceramic/Color/)> | *(optional)* | Optional array to store results in. If provided, it will be reused; otherwise a new array is created  |
