...
title: MeshUtils
target: Unity
permalink: api-docs/unity/ceramic/MeshUtils/
---
...
|------|------|---------|-------------|
| `vertices` | [Array](/api-docs/unity/Array/)<[Float](/api-docs/unity/Float/)> | *(optional)* | Existing array to reuse, or null to create new array. If provided and larger than needed, will be truncated.  |
...
| `staggerX` | [Float](/api-docs/unity/Float/) | `0` | Horizontal offset applied to odd-numbered rows. Use cellWidth*0.5 for hexagonal grids.  |
| `staggerY` | [Float](/api-docs/unity/Float/) | `0` | Vertical offset applied to odd-numbered columns. Rarely used, but can create diamond patterns.  |
| `attrLength` | [Int](/api-docs/unity/Int/) | `0` | Number of custom float attributes per vertex. Common values: 2 for UV, 4 for color, etc.  |
| `attrValues` | [Array](/api-docs/unity/Array/)<[Float](/api-docs/unity/Float/)> | *(optional)* | Array of attribute values to assign. If null, attributes are initialized to 0. Length must equal (columns+1)×(rows+1)×attrLength.  |

| Returns | Description |
|---------|-------------|
| [Array](/api-docs/unity/Array/)<[Float](/api-docs/unity/Float/)> | Array of vertex data with length (columns+1)×(rows+1)×(2+attrLength) * ```haxe // Simple 10x10 grid var vertices = MeshUtils.createVerticesGrid(null, 10, 10, 400, 400); * // Hexagonal grid with horizontal stagger var cellWidth = 40; var vertices = MeshUtils.createVerticesGrid( null, 10, 10, 400, 400, cellWidth * 0.5, 0 ); * // Grid with per-vertex colors (4 floats: r,g,b,a) var colors = [ // color data ]; var vertices = MeshUtils.createVerticesGrid( null, 10, 10, 400, 400, 0, 0, 4, colors ); ``` |
...
| Name | Type | Default | Description |
|------|------|---------|-------------|
| `indices` | [Array](/api-docs/unity/Array/)<[Int](/api-docs/unity/Int/)> | *(optional)* | Existing array to reuse, or null to create new array. If provided and larger than needed, will be truncated.  |
| `columns` | [Int](/api-docs/unity/Int/) | | Number of columns in the grid (cells, not vertices)  |
| `rows` | [Int](/api-docs/unity/Int/) | | Number of rows in the grid (cells, not vertices)  |
| `mirrorX` | [Bool](/api-docs/unity/Bool/) | `false` | Mirror triangle diagonal in odd-numbered columns. Creates horizontal alternation pattern.  |
| `mirrorY` | [Bool](/api-docs/unity/Bool/) | `false` | Mirror triangle diagonal in odd-numbered rows. Creates vertical alternation pattern.  |
| `mirrorFlip` | [Bool](/api-docs/unity/Bool/) | `false` | Inverts the mirroring pattern. If true, mirrors even cells instead of odd.  |

| Returns | Description |
|---------|-------------|
| [Array](/api-docs/unity/Array/)<[Int](/api-docs/unity/Int/)> | Array of indices with length columns×rows×6 (2 triangles × 3 vertices per cell) * ```haxe // Standard grid triangulation var indices = MeshUtils.createIndicesGrid(null, 10, 10); * // Alternating pattern for natural terrain var indices = MeshUtils.createIndicesGrid( null, 10, 10, true, true // Mirror both X and Y ); * // Custom pattern with flipped mirroring var indices = MeshUtils.createIndicesGrid( null, 10, 10, true, false, true // Mirror X on even columns ); ``` |
...
| Name | Type | Default | Description |
|------|------|---------|-------------|
| `uvs` | [Array](/api-docs/unity/Array/)<[Float](/api-docs/unity/Float/)> | *(optional)* | Existing array to reuse, or null to create new array. If provided and larger than needed, will be truncated.  |
| `columns` | [Int](/api-docs/unity/Int/) | | Number of columns in the grid (cells, not vertices)  |
| `rows` | [Int](/api-docs/unity/Int/) | | Number of rows in the grid (cells, not vertices)  |
| `offsetX` | [Float](/api-docs/unity/Float/) | `0` | Horizontal UV offset for texture scrolling. Values > 1 create tiling if texture wrap is enabled.  |
| `offsetY` | [Float](/api-docs/unity/Float/) | `0` | Vertical UV offset for texture scrolling. Values > 1 create tiling if texture wrap is enabled.  |
