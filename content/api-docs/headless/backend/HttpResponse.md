...
title: HttpResponse
target: Headless
permalink: api-docs/headless/backend/HttpResponse/
---
...
|------|------|---------|-------------|
| `status` | [Int](/api-docs/headless/Int/) | | HTTP status code returned by the server (e.g., 200, 404, 500) |
| `content` | [String](/api-docs/headless/String/) | | * Response content as a string for text responses. This may contain trailing newlines that are cleaned up by higher-level processing. |
| `binaryContent` | [Null](/api-docs/headless/Null/)<[haxe.io.Bytes](/api-docs/headless/haxe/io/Bytes/)> | *(optional)* | * Response content as raw bytes for binary responses. Defaults to null for text responses. |
| `error` | [Null](/api-docs/headless/Null/)<[String](/api-docs/headless/String/)> | *(optional)* | * Error message for failed requests. Defaults to null for successful requests. |
| `headers` | [Array](/api-docs/headless/Array/)<[String](/api-docs/headless/String/)> | | * HTTP response headers as a flat array: [key1, value1, key2, value2, ...]. This format allows multiple headers with the same name. Header names may not be normalized at this level. |
